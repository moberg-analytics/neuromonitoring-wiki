...
# Notebook 5 — Data Modelling & Visualization

**For more information or help, please email support@moberganalytics.com**

In this notebook, we'll explore applying and developing models on real ICU monitoring data. We'll use existing pretrained models for artifact detection and time series forecasting, then build and evaluate new models for predicting patient outcomes. The goal is to understand the typical modelling workflow from signal cleaning through prediction so participants can recognize the steps involved in analyzing neurocritical care data.

We will go over 3 self-contained sections, each of which showcases a different modelling application and can be run independently.

### What you'll learn

* How to identify and visualize waveform artifacts in Arterial Blood Pressure (ABP) and Electrocardiogram (ECG) waveforms using pretrained models.
* How to forecast Intracranial Pressure (ICP) using a compact recurrent model (LSTM).
* How to build and evaluate models for predicting patient outcomes.

## Setup

In this section, we'll import the necessary libraries and set up the environment. We'll be using a number of third-party libraries for data manipulation, visualization and machine learning. We'll also be using some of our internal libraries for interacting with CNS archives (`cns_utiils`), digital signal processing (`moberg_dsp`) and machine learning utilities (`sam_tools`).
...
## Cleaning Waveform Data (ABP & ECG)
...
<img src="./images/artifact-detection-overview.png" width="60%" style="display: block; margin-left: auto; margin-right: auto" />
...
You may observe that some ECG artifacts exhibit significantly higher amplitude than the surrounding signal. We can modify the y-axis scale to better visualize the waveform.
...
<img src="./images/cbr-algorithm.png" width="60%" style="display: block; margin-left: auto; margin-right: auto" />
...
We'll start byt loading the dataset
